...
##  🧠 Abstract
<p align="justify">
   <strong>Abstract:</strong> Accurate prediction of drug–target interactions (DTIs) remains challenging due to the multi-scale and heterogeneous nature of molecular recognition. We present UniDTI, a unified deep learning framework that integrates multi-modal representations of drugs and proteins with a Bidirectional Intention Network (BIN) for dynamic interaction modeling. UniDTI combines molecular sequences, hierarchical graphs, physicochemical descriptors, protein pocket graphs, and sequence motifs to capture complex ligand–target relationships across modalities and spatial scales. Across multiple benchmark datasets, including BindingDB, BioSNAP, Davis, DRH, and GLASS, UniDTI consistently outperforms state-of-the-art methods and shows strong generalization under cold-start and low-data settings. In virtual screening against the μ-opioid receptor, UniDTI further identified two novel active compounds with experimental validation, demonstrating its potential for AI-driven drug discovery.
</p>
